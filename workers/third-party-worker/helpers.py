...
def find_playlists(lyrics, audio_features, additional_features, df):
    ...
    system_prompt = THIRD_PARTY_PLAYLIST_PROMPT
    ...
    df_sorted = df.sort_values("tag_similarity", ascending=False)

    valid_playlists = []
    index = 0

    while len(valid_playlists) < 3 and index < len(df_sorted):
        playlist = df_sorted.iloc[
            random.randint(index, min(index + 9, len(df_sorted) - 1))
        ]

        playlist_id = playlist["URL Full"].split("/")[-1]
...
def write_pitch(
    playlist_title, playlist_tags, lyrics, audio_features, additional_features, track_title
) -> str:
...
def get_third_party_playlists(lyrics, audio_features, additional_features, df, track_title) -> dict:
    print("Starting get_third_party_playlists")

    print("Finding third_party playlists")
    playlist_infos = find_playlists(lyrics, audio_features, additional_features, df)
    print(f"Found {len(playlist_infos)} third_party playlists")
    ...
    print(f"Retrieved {len(result)} third_party playlist objects")
    return result
