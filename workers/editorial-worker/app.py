...
import sys
import pandas as pd
from helpers import get_editorial_playlists
import traceback

load_dotenv(override=True)

app = Flask(__name__)
df = pd.read_csv("editorial.csv")
...
@app.route("/editorial-playlists", methods=["POST"])
def editorial_playlists_endpoint():
    ...
    try:
        ...
        editorial_playlists = get_editorial_playlists(
            lyrics, audio_features, additional_features, df, track_title
        )

        return jsonify(editorial_playlists), 200
