...
from helpers import get_notable_moments
import sys
import traceback
...
def test_route():
    ...
    return "notable moments worker is live"


@app.route("/notable-moments", methods=["POST"])
def notable_moments_endpoint():

    transcription = request.json.get("transcription")

    try:

        notable_moments = get_notable_moments(transcription)

        return jsonify(notable_moments), 200
