...
from helpers import get_key_traits
import sys
import traceback
...
def test_route():
    ...
    return "key traits worker is live"


@app.route("/key-traits", methods=["POST"])
def key_traits_endpoint():

    lyrics = request.json.get("lyrics")
    audio_features = request.json.get("audioFeatures")
    quantized_audio_features = request.json.get("quantizedAudioFeatures")
    additional_features = request.json.get("additionalFeatures")

    try:

        key_traits = get_key_traits(
            lyrics,
            audio_features,
            quantized_audio_features,
            additional_features,
        )

        return jsonify(key_traits), 200
