...
export const draw = (
    ...
    const highlightStartPercent = highlightStart
        ? highlightStart / duration
        : 0;
    const highlightEndPercent = highlightEnd ? highlightEnd / duration : 1;

    data.forEach((dp, i) => {
        const mappingPercent = i / data.length;
        const played = playedPercent > mappingPercent;
        const inHighlightRange =
            mappingPercent >= highlightStartPercent &&
            mappingPercent <= highlightEndPercent;

        if (inHighlightRange && highlightColor) {
            ctx.fillStyle = highlightColor;
        } else {
            ctx.fillStyle =
                played && barPlayedColor ? barPlayedColor : barColor;
        }
